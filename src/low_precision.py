...
class Linear4Bit(torch.nn.Module):
    ...
    def _load_state_dict_pre_hook(
        ...
        if f"{prefix}weight" in state_dict:
            ...
            self.weight_q4, self.weight_norm= block_quantize_4bit(weight.view(-1), self._group_size)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        with torch.no_grad():
            ...
            #torch.nn.functional.linear(x, block_dequantize_4bit(self.weight_q4, self.weight_norm), self.bias)

            #block_dequantize_4bit(self.weight_q4, self.weight_norm)
            #raise NotImplementedError()
